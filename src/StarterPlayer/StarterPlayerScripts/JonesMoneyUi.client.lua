...
local function stylePanel(panel: Frame)
...
local function styleCard(card: Frame, themeColor: Color3)
	card.BackgroundColor3 = Color3.fromRGB(
		math.clamp(math.floor(themeColor.R * 255 * 0.22 + 42), 0, 255),
		math.clamp(math.floor(themeColor.G * 255 * 0.22 + 42), 0, 255),
		math.clamp(math.floor(themeColor.B * 255 * 0.22 + 46), 0, 255)
	)
...
local MODAL_MAX_WIDTH = 560
local MODAL_WIDTH_SCALE = 0.92
local MODAL_HEIGHT_SCALE = 0.78
local MODAL_SAFE_MARGIN = 24
local MODAL_MIN_WIDTH = 280
local MODAL_MIN_HEIGHT = 220
local MODAL_HEADER_HEIGHT = 44
local FOOD_ACTION_BUTTON_WIDTH = 64
local FOOD_ACTION_BUTTON_HEIGHT = 44
local FOOD_GRID_GAP = 10
local FOOD_CARD_HEIGHT = 224
...
local function createCenteredModal(modalName: string, titleText: string, titleColor: Color3): ModalController
	...
	titleLabel.Size = UDim2.new(1, -FOOD_ACTION_BUTTON_WIDTH - 8, 1, 0)
	...
	closeButton.Size = UDim2.fromOffset(FOOD_ACTION_BUTTON_WIDTH, FOOD_ACTION_BUTTON_HEIGHT)
	...
	applyButtonHover(closeButton, Color3.fromRGB(70, 70, 80), Color3.fromRGB(95, 95, 110))
...
local foodInventoryCards: { [string]: { card: Frame, countLabel: TextLabel } } = {}
...
local function createFoodThumbnailArea(parent: Frame, layoutOrder: number, item: typeof(FOOD_SHOP_DISPLAY_ITEMS[1])): Frame
	...
	thumbFrame.Size = UDim2.new(1, 0, 0, 56)
	thumbFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 24)
	...
	thumbImage.Size = UDim2.fromOffset(44, 44)
	thumbImage.AnchorPoint = Vector2.new(0.5, 0.5)
	thumbImage.Position = UDim2.fromScale(0.5, 0.5)
...
local foodShopModal = createCenteredModal("FoodShopModal", "Food Shop", Color3.fromRGB(255, 220, 160))
...
for index, item in FOOD_SHOP_DISPLAY_ITEMS do
	...
	local ownedLabel = createCardMetaLabel(card, 3, "Owned: 0", 12, Color3.fromRGB(160, 200, 255))
	ownedLabel.Name = "OwnedLabel"
	foodShopOwnedLabels[item.Id] = ownedLabel

	createCardMetaLabel(card, 4, ("Price: %d"):format(item.Price), 12, Color3.fromRGB(255, 220, 120))
	createCardMetaLabel(card, 5, ("+%d Energy"):format(item.EnergyRestore), 12, Color3.fromRGB(120, 220, 140))
	createCardMetaLabel(card, 6, ("+%d Hunger"):format(item.HungerRestore), 12, Color3.fromRGB(255, 170, 100))
	createCardMetaLabel(card, 7, ("%d min"):format(item.FullnessMinutes), 12, Color3.fromRGB(200, 200, 210))

	local buyItemButton = Instance.new("TextButton")
	buyItemButton.Name = "BuyButton"
	buyItemButton.LayoutOrder = 8
	...
	applyButtonHover(buyItemButton, Color3.fromRGB(180, 110, 40), Color3.fromRGB(210, 140, 60))
...
local foodInventoryModal = createCenteredModal("FoodInventoryModal", "Food Inventory", Color3.fromRGB(210, 190, 255))
...
for index, item in FOOD_SHOP_DISPLAY_ITEMS do
	...
	local countLabel = createCardMetaLabel(card, 3, "x0", 13, Color3.fromRGB(160, 200, 255))
	countLabel.Name = "CountLabel"
	countLabel.Font = Enum.Font.GothamMedium

	createCardMetaLabel(card, 4, ("+%d Energy"):format(item.EnergyRestore), 12, Color3.fromRGB(120, 220, 140))
	createCardMetaLabel(card, 5, ("+%d Hunger"):format(item.HungerRestore), 12, Color3.fromRGB(255, 170, 100))
	createCardMetaLabel(card, 6, ("%d min"):format(item.FullnessMinutes), 12, Color3.fromRGB(200, 200, 210))
	...
	applyButtonHover(eatButton, Color3.fromRGB(70, 130, 95), Color3.fromRGB(90, 160, 115))

	eatButton.MouseButton1Click:Connect(function()
		performActionRemote:FireServer("EatFoodItem", item.Id)
	end)

	foodInventoryCards[item.Id] = { card = card, countLabel = countLabel }
end
